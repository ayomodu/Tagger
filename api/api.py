...
def add_project(item: Item):
    ...
    try:
        ...
        if development_version != None:
            cursor.execute(f'insert into tagger (project_name,development) values("{project_name}", "{development_version}") on conflict(project_name) do update set development=excluded.development')
            return {"Success": f"Project {project_name} development version updated to: {development_version}"}
        if staging_version != None:
            cursor.execute(f'insert into tagger (project_name,staging) values("{project_name}", "{staging_version}") on conflict(project_name) do update set staging=excluded.staging')
            return {"Success": f"Project {project_name} staging version updated to: {staging_version}"}
        if production_version != None:
            cursor.execute(f'insert into tagger (project_name,production) values("{project_name}", "{production_version}") on conflict(project_name) do update set production=excluded.production')
